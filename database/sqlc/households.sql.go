...
SELECT household_id, num_people, name FROM households
WHERE household_id = $1
`
...
SELECT id, household_id, invite_code, requested_by_user_id, status, created_at FROM household_invites
WHERE invite_code = $1
`
...
const getPendingInvitesForHousehold = `-- name: GetPendingInvitesForHousehold :many
...
VALUES ($1, $2)
RETURNING household_id, num_people, name
`
